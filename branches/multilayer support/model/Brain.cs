    ...
    class Brain
    {
        ...
        public void think()
        {
            ...
            if (this.isInLearnMode)
            {
                ...
                {
                    if (this.checkForRecentlyFiredNeuronsWithoutCommonSuccessor(l))
                    {
                        Neuron tmpNeuron = this.associateLastFiredNeuronsWithNewNeuron(l);
        ...
        private bool checkForRecentlyFiredNeuronsWithoutCommonSuccessor(int cLayer)
        {
            Neuron n;
            ...
            while (n.layer < cLayer)
            {
                //if (n.layer == 0) elements ++;
                if ((n = recoveryQueue.getNext(0))==null) return false;
            }
            elements++;
            ...
            if (elements < 2 && cLayer != 0) return false;
        ...
        private Neuron associateLastFiredNeuronsWithNewNeuron(int cLayer)
        {
            Neuron tmpNeuron = new Neuron(this);
            //tmpNeuron.lastFired = currentStep+1;
            tmpNeuron.layer = cLayer + 1;
            if (tmpNeuron.layer == this.maxLayer) tmpNeuron.layer = cLayer;
            Neuron n;
            while ((n = recoveryQueue.getNext()) != null)
            {
                if (n.layer >= cLayer)
