...
if (mysql_num_rows($result) > 0) {
  $erg = mysql_fetch_array($result,MYSQL_ASSOC);
  //All die Reaktionen (suc) auswählen, auf die absolut eine positive Rückmeldung erfolgte.
  ...
  $query2 = "SELECT *, COUNT(suc) AS cdest FROM cortex2 WHERE dest=" . $erg["relid"] . " AND suc > 0 Group By suc ORDER BY cdest DESC";
  debug($query2);
  $result2 = mysql_query($query2,$db);
  for ($mc=0;$mc < mysql_num_rows($result2) && $foundrels[0] == 0;$mc++) {
    $rels = mysql_fetch_array($result2,MYSQL_ASSOC);
    //... Davon die auswählen, bei denen in Bezug a d Gesagte durchschnittlich positive Rückmeldungen kamen.
    $query3 = "SELECT AVG(pos) as posavg FROM cortex2 WHERE dest=" . $rels["suc"] ." AND relid = " . $rels["relid"]; 
    debug ($query3);
