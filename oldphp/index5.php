...
while ($activeneurons == 1) {
  ...
  if(mysql_num_rows($resultx)>0) {  
    ...
    $query1 = "SELECT delay, suc, strength as synapsen FROM axons WHERE dest=" . $relx["id"];
    $result = mysql_query($query1,$db);
    if (mysql_num_rows($result) > 0) {
      for ($y=0;$y<mysql_num_rows($result);$y++) {
        $erg = mysql_fetch_array($result,MYSQL_ASSOC);
        if ($erg["suc"] != 0) {
  	 //$query1 = "SELECT suc, sum(strength) as synapsen, delay FROM axons GROUP BY suc HAVING suc=". $erg["suc"];
	 $query1 = "SELECT sum(strength) as synapsenTotal FROM axons WHERE dest=" . $relx["id"];	
	 //echo $query1 . "<br>";
	 $result2 = mysql_query($query1,$db); 
  	 $erg2 = mysql_fetch_array($result2,MYSQL_ASSOC);
  	 // Synfaktor ist die Anzahl der Synapsen (akt. Verbindung) durch die gesamtzahl der synpasen am zielneuron
  	 $synfaktor = $erg["synapsen"] / $erg2["synapsenTotal"];
  	 $query1 = "INSERT INTO activationQueue (neuronid, activationVal, activationDelay) VALUES (" . $erg["suc"] . ", " . mysql_num_rows($result2) * $synfaktor . "," . $erg["delay"] . ")";
  ...
  //$result = mysql_query($query1,$db);
    ...
    //Hierarchie wieder nach unten gehen
...
?>
<form name=mouth action="index5.php" method="get">
