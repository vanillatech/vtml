...
//change index16.php: Newneuron - neuer lernalgorithmus
...
function newNeuron () {
    ...
    while ($again == 1) { 
    $again = 0;
    //unterste schicht in recQueue ermitteln
    $query1 = "SELECT MIN(layer) AS ml from recoveryQueue";
	...
	for ($y = 0; $y < $numlastfired;$y++) {
		...
		if ($y == 0) {
			// neues Neuron ist eine Schicht höher ("Columnar Arrangements") als unterste Schicht in recQueue (Annahme)
			$cnewlayer = $min + 1;
