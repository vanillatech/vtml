...
//change index14i.php: thinkdown ordnen nach oberster schicht + aus recoveryqueue alles rauslöschen 
//change index15.php: deleRecQueue erkennt unter Umständen falsch - gefixed.
//change index15a.php: debug an/abstellen
//change index17.php: newneuron - anderer algorithmus
//change index17a.php: Fehler in index17 behoben: neues Neuron an falscher fireorder eingefügt
//change index17b.php: newneuron - nach jeder "Pause" (innerhalb einer Schicht) ein neues Neuron einfügen
//change index17c.php: Test: recoveryQueue am Ende nicht löschen
//change index17d.php: newneuron: neues neuron nicht in recQueue + 17c rückgängig
...
//change index19d.php: replaceStruct: neue Funktion zum objektorientierten Ersetzen
//change index19e.php: alles vergessen, was nicht gleich (innerhalb recTime) wiederholt wurde.
//change index19f.php: thinkdown: index19c rückgängig.
...
function newNeuron () {
	...
	//if ($clayer != $erg["layer"]) {
		...
		$cnewlayer = $clayer + 1;
		$query1 = "INSERT INTO neurons (lastfired,recoveryTime,layer) values (0,$recTime,$cnewlayer)";
		$result = mysql_query($query1,$db);
		$newneuron = mysql_insert_id();	
		//recoveryQueue zeitlich rückwärts geordnet - neues Neuron ist 1 später als letztes Ereignis auf unterer Schicht
...
function replaceStruct(&$fNeuron,$pos,$forderOld) {
    global $db;
    global $verboseX;
    $fQuery = "";
    if ((count($fNeuron)) == $pos) return(1);
    ...
    if ($pos > 0) {
      ...
      $forderNew = $forderOld - $delay;
      if ($forderNew < 0) return(0);
      $fQuery = " AND delay = " . $forderNew;
    } 
    $query1 = "SELECT * FROM axons WHERE dest = " . $fNeuron[$pos]["dest"] . $fQuery . " AND suc <> " . $fNeuron[$pos]["suc"] . " ORDER BY delay DESC";
    $result2 = mysql_query($query1,$db);
    for ($y=0;$y<mysql_num_rows($result2);$y++) {
      $res2 = mysql_fetch_array($result2,MYSQL_ASSOC);
      $retVal = replaceStruct($fNeuron,$pos+1,$res2["delay"]);
      if ($retVal == 1) {
	if ((count($fNeuron)) == $pos + 1) {
		$query1 = "INSERT INTO axons (dest,suc,delay) values (". $fNeuron[$pos]["suc"] ."," . $res2["suc"] . "," . ($res2["delay"] - 1) . ")";
	        $result2 = mysql_query($query1,$db);
	}
        $query1 = "DELETE FROM axons WHERE id = " . $res2["id"];
	$result = mysql_query($query1,$db);
	$verboseX .= "<b><font color=#FFCCCC>Delete From Axons: </font></b> NeuronID: " . $fNeuron[$pos]["dest"] . " Successor: " . $fNeuron[$pos]["suc"] . "<br>";
	return(1);
      } else { return(0); } 
    } 
    if (mysql_num_rows($result2) == 0) return(0);
...
// alles vergessen, was nicht innerhalb der recTime wiederholt wurde

  $query1 = "DELETE FROM neurons WHERE recoveryTime = 0 AND id NOT IN (SELECT suc FROM axons WHERE strength > 1 UNION SELECT dest AS suc FROM axons WHERE strength > 1) AND id NOT IN (SELECT neuronid FROM activationQueue)";
  $result = mysql_query($query1,$db);
  $query1 = "DELETE FROM axons WHERE dest NOT IN (SELECT id FROM neurons) OR suc NOT IN (SELECT id FROM neurons)";
  $result = mysql_query($query1,$db);
...
while ($activeneurons == 1) {
  ...
  if(mysql_num_rows($resultx)>0) {  
    ...
    if (mysql_num_rows($result) > 0) {
	    ...
	    if ($a == 1) { //wurde gefunden
	      $a = replaceStruct($fNeuron,0,0);
	    }
    }	   
    unset($fNeuron); 
...
function thinkDown ($neuronID) {
  ...
  if (mysql_num_rows($result)>0) {
	  ...
	  $strongest = $erg["strongest"];
  	  $query1 = "SELECT axons.*, neurons.id, neurons.layer from axons, neurons where axons.suc = $neuronID and neurons.id = axons.dest ORDER BY delay DESC";	
 	  $result = mysql_query($query1,$db);
	  if (mysql_errno())
...
$query1 = "UPDATE neurons set active = 0, lastfired = 0";
$result = mysql_query($query1,$db);
