...
$query1 = "UPDATE cortex0_0 SET active = (active * $forgetrate)";
$result = mysql_query($query1,$db);
$query1 = "UPDATE cortex0 SET active = (active * $forgetrate)";
$result = mysql_query($query1,$db);
$query1 = "UPDATE cortex2 SET active = (active * $forgetrate)";
$result = mysql_query($query1,$db);
$query1 = "UPDATE relations SET active = (active * $forgetrate)";
$result = mysql_query($query1,$db);
  //1.2 nun durch die äußeren Reize (gehörte Laute/Buchstaben) Neuronen und ihre Umgebung reizen.
  $query1 = "SELECT relid FROM cortex0 WHERE dest=" . $charsid[$l];
  $result = mysql_query($query1,$db);
  if (mysql_num_rows($result) > 0) {
    $query2 = "UPDATE cortex0 set active = (active + 1/(" . mysql_num_rows($result) . "*2)) WHERE dest=" . $charsid[$l];
    $result2 = mysql_query($query2,$db);
    for ($y=0;$y<mysql_num_rows($result);$y++) {
      $erg = mysql_fetch_array($result,MYSQL_ASSOC);
      if ($erg["suc"] != 0) {
	 $query1 = "UPDATE cortex0 set active = (active + 1/(" . mysql_num_rows($result) . "*2)) WHERE dest=". $erg["suc"];
