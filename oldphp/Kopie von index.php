...
if (mysql_num_rows($result) != 0) {
  ...
  $relid++;
  for ($n=0;$n<count($words);$n++) {
    $query1 = "INSERT INTO relations(relid,dest,suc) values ($relid,'" . $wordsid[$n] . "','" . $wordsid[$n+1] . "')";
    $result = mysql_query($query1,$db);
  }
...
$query1 = "SELECT relid, COUNT(relid) AS ord FROM relations WHERE $notuserecent (";
for ($n=0;$n<count($words);$n++) {
...
$query1 .= ") GROUP BY relid ORDER BY ord DESC";
$result = mysql_query($query1,$db);
if (mysql_num_rows($result) > 0) {
  $erg = mysql_fetch_array($result,MYSQL_ASSOC);
