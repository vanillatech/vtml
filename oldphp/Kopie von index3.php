...
for ($n=0;$n<count($words);$n++) {
  ...
  for ($l=0;$l<strlen($words[$n]);$l++) {
	$query1 = "SELECT * FROM neurons WHERE data like '$aword[$l]'";
	$result = mysql_query($query1,$db);
	if (mysql_num_rows($result) == 0) {
		$query2 = "INSERT INTO neurons(data) values ('$aword[$l]')";
  ...
  //1. Behandlung der Aktivität der Neuronen.
  //1.1 Erstmal wieder "abregen":
$query1 = "UPDATE neurons SET active = (active * $forgetrate)";
$result = mysql_query($query1,$db);
  //1.2 nun durch die äußeren Reize (gehörte Laute/Buchstaben) Neuronen und ihre Umgebung reizen.
  $query1 = "SELECT suc FROM axons WHERE dest=" . $charsid[$l];
  $result = mysql_query($query1,$db);
  if (mysql_num_rows($result) > 0) {
    $query2 = "UPDATE neurons set active = (active + 1/(" . mysql_num_rows($result) . "+1)) WHERE dest=" . $charsid[$l];
    $result2 = mysql_query($query2,$db);
    for ($y=0;$y<mysql_num_rows($result);$y++) {
      $erg = mysql_fetch_array($result,MYSQL_ASSOC);
      if ($erg["suc"] != 0) {
	 $query1 = "UPDATE neurons set active = (active + 1/(" . mysql_num_rows($result) . "+1)) WHERE dest=". $erg["suc"];
...
$charrelid = 0;
  ...
  for ($l=0;$l<strlen($words[$n]);$l++) {
    if ($l!=0) $query1 .= " AND neurons.id IN ( ";
    $query1 .= "SELECT neurons.id as id FROM neurons, axons WHERE axons.dest=" . $charsid[$l] . " AND axons.suc=neurons.id";
    if ($l!=0) $query1 .= ")";
  }
  $result = mysql_query($query1,$db);
  if (mysql_num_rows($result) > 0) {
...
?>
<form name=mouth action="index2.php" method="get">
