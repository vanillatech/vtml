...
$charrelid = 0;
  ...
  if (mysql_num_rows($result) > 0) {
    ...
    $notuseRword = "b.relid <> " . $charrelid . " AND";  
  ...
  for ($l=0;$l<strlen($words[$n]);$l++) {
    if ($l != 0) $query1 .= " AND b.suc=$charsid[$l] OR ";
    $query1 .= "b.dest=$charsid[$l]";
  }
...
if (mysql_num_rows($result) != 0) {
  ...
  $notuserecent = "b.relid <> " . $relid . " AND";
}
...
/*for ($m=0;$m<count($words);$m++) {
  ...
  if ($n != 0) $query1 .= "OR ";
  $query1 .= "b.dest=$wordsid[$n] "; //OR suc=$wordsid[$n] ";
}
