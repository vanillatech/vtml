...
//change index13b.php: Hebbsche synaptische Plastizität bereits beim Aktivieren.
...
function newNeuron () {
	...
	//$query1 = "INSERT INTO axons (dest,suc) values ($lastspikeid," . $relx["id"] . ")";	  
		...
		//if ($y == 0) $fireord = $erg["fireorder"];
	        $sumDelay += $erg["fireorder"] - $fireord;
...
while ($activationQueue == 1) {
  ...
  if(mysql_num_rows($resultx)>0) {
    for ($g = 0;$g < mysql_num_rows($resultx);$g++) {
      ...
      //Hebbsches Lernen - Synaptische Plastizität:
      $query1 = "UPDATE axons set strength = (strength + 1) WHERE dest = " . $erg["sender"];
...
while ($activeneurons == 1) {
  ...
  if(mysql_num_rows($resultx)>0) {  
    ...
    $query1 = "SELECT * FROM neurons WHERE id IN ( SELECT suc FROM axons WHERE dest IN (SELECT neuronid FROM recoveryQueue WHERE neuronid IN (SELECT dest FROM axons WHERE suc = " . $relx["id"] . ")))";
    $result = mysql_query($query1,$db);
