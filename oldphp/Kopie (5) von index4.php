...
if (mysql_affected_rows($db) == 0) $stepbystep = 0;
  ...
  if(mysql_num_rows($resultx)>0) {  
    ...
    if (mysql_num_rows($result) > 0) {
	 ...
	 $result2 = mysql_query($query1,$db); 
  	 ...
  	 // Synfaktor ist die Anzahl der Synapsen (akt. Verbindung) durch die gesamtzahl der synpasen am zielneuron
  	 $synfaktor = $erg["synapsen"] / $erg2["synapsen"];
  	 $query1 = "INSERT INTO activationQueue (neuronid, activationVal, activationDelay) VALUES (" . $erg["suc"] . ", " . (1 / mysql_num_rows($result)) * $synfaktor . "," . $erg["delay"] . ")";
...
$neuronsleft = 1;    
//while ($neuronsleft > 0) {
	$query1 = "SELECT * from neurons where lastfired > 0 order by lastfired DESC";
	$result = mysql_query($query1,$db);
	if (mysql_num_rows($result)>0) {
	  $erg = mysql_fetch_array($result,MYSQL_ASSOC);	
	  thinkDown($erg["id"]);
...
$query1 = "UPDATE neurons set recoveryTime = 0, lastfired = 0";
$result = mysql_query($query1,$db);
