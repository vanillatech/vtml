...
while ($activeneurons == 1) {
  ...
  if(mysql_num_rows($resultx)>0) {  
    ...
    $query1 = "UPDATE neurons set recoveryTime = 0 WHERE id IN (SELECT dest FROM axons WHERE suc = " . $relx["id"] . " AND dest IN (SELECT id FROM neurons WHERE recoveryTime > 0))";
    $result = mysql_query($query1,$db);
    ...
    $query1 = "SELECT dest, sum(strength) as synapsen FROM axons group by dest having dest=" . $relx["id"];
    $result = mysql_query($query1,$db);
    if (mysql_num_rows($result) > 0) {
      $erg3 = mysql_fetch_array($result,MYSQL_ASSOC);
      $synapsendest = $erg3["synapsen"];
	 ...
	 $result2 = mysql_query($query1,$db); 
  	 $erg2 = mysql_fetch_array($result2,MYSQL_ASSOC);
  	 // Synfaktor ist die Anzahl der Synapsen (akt. Verbindung) durch die gesamtzahl der synpasen am zielneuron
  	 $synfaktor = $erg["synapsen"] / ($erg2["synapsen"] * $synapsendest);
